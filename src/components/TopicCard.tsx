...
// Props do card de tópico
interface TopicCardProps {
  topic: Topic
  progress: ReturnType<typeof useProgress>
}

// Labels e cores dos níveis
const levelConfig = {
  junior: { label: 'Júnior', className: 'bg-emerald-500/15 text-emerald-400 border border-emerald-500/20' },
  mid: { label: 'Pleno', className: 'bg-amber-500/15 text-amber-400 border border-amber-500/20' },
  senior: { label: 'Sênior', className: 'bg-red-500/15 text-red-400 border border-red-500/20' },
}
...
export function TopicCard({ topic, progress }: TopicCardProps) {
  ...
  const completed = progress.isCompleted(topic.id)
  const noteContent = progress.getNote(topic.id)

  // Alterna conclusão do tópico
  const handleToggle = useCallback(() => {
    void progress.toggleTopic(topic.id)
  }, [progress, topic.id])
  ...
  const handleNoteChange = useCallback(
    (e: React.ChangeEvent<HTMLTextAreaElement>) => {
      ...
      saveTimeoutRef.current = setTimeout(() => {
        void progress.saveNote(topic.id, content)
      }, 800)
    },
    [progress, topic.id]
  )
    ...
    >
      ...
      <div className="flex items-start gap-3 p-4">
        ...
        <button
          ...
          aria-label={completed ? `Marcar ${topic.title} como incompleto` : `Marcar ${topic.title} como completo`}
        >
          {completed && (
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${level.className}`}>
              {level.label}
            </span>
            <span className={`text-xs font-medium ${difficulty.className}`}>
              {difficulty.label}
            </span>
      ...
      {expanded && (
        <div className="border-t border-zinc-800 px-4 py-4 space-y-4">
          ...
          <div>
            ...
            <p className="text-sm text-zinc-400 leading-relaxed">{topic.description}</p>
          </div>
          ...
          <div>
            ...
            <p className="text-sm text-zinc-400 leading-relaxed">{topic.whyItMatters}</p>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-1.5">
              {topic.mustKnowWithout.map((item, i) => (
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Adicione suas anotações, insights ou dúvidas sobre este tópico..."
              rows={3}
